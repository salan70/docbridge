...
function request<T>(method: string, params: unknown): Promise<T> {
  ...
  return new Promise((res) => {
    pending.set(id, (result) => res(result as T));
...
child.stdout.on("data", (chunk: Buffer) => {
  ...
  for (;;) {
    ...
    const resolvePending = typeof msg.id === "number" ? pending.get(msg.id) : undefined;
...
const open = (rel: string, text: string) =>
  send({
    ...
    params: {
      textDocument: {
        uri: uri(rel),
        languageId: rel.endsWith(".ts") ? "typescript" : "markdown",
        version: 1,
        text,
      },
    },
...
const hover = await request<{ contents?: { value?: string } } | null>("textDocument/hover", {
...
const def = await request<{ uri?: string } | { uri?: string }[] | null>("textDocument/definition", {
  textDocument: { uri: uri("src/lsp/server.ts") },
  position: pos,
});
...
const refs = await request<unknown[]>("textDocument/references", {
  textDocument: { uri: uri("src/lsp/server.ts") },
  position: pos,
});
check(
