...
description: Review all existing SpecLink links for semantic validity. Use after adoption or periodically to find valid-but-wrong links, stale docs, excessive links, or ambiguous docs/code relationships.
---

# speclink-review

Review the semantic validity of the entire SpecLink graph. This is not a
...
This skill requires `speclink graph --json --include-content`.
...
1. **Build the graph.**
   ...
   `diagnostics` first. Mechanical failures from `speclink check` should be
...
5. **Report findings and fixes.** For each finding include:
   ...
   `speclink-link` or `speclink-annotate` rules to make the change.

## Principles

- `speclink check` proves link mechanics; this skill reviews meaning.
