---
name: speclink-sync
description: Triage SpecLink related-gate findings — linked counterparts of changed files that were not themselves updated. Use when a Stop-hook message, CI comment, or speclink related --gate run flags unchanged counterparts, to judge divergence with speclink context and then update or justify each one.
---

# speclink-sync

Resolve `speclink related --gate` findings. A gate violation means a changed
...
1. **Collect the violations.** Use the gate report you were given (Stop-hook
   ...
   ```sh
   git diff --name-only HEAD | speclink related --stdin --gate
   ```
...
3. **Judge divergence, per counterpart.** Compare what the counterpart says
   ...
   - The link itself is wrong (points at the wrong section or symbol) → fix
     the annotation pair instead; follow the `speclink-annotate` skill.
